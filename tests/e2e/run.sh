...
case "$profile" in
  test|test-sb|test-tm) profiles=("$profile") ;;
  all) profiles=(test test-sb test-tm) ;;
  *) echo "usage: $0 {test|test-sb|test-tm|all} [run|verify]" >&2; exit 2 ;;
esac
...
if [[ "$mode" != verify ]]; then
  ...
  for selected in "${profiles[@]}"; do
    "$nextflow_bin" run "$root" -profile "$selected,$engine" -ansi-log false
  done
fi
