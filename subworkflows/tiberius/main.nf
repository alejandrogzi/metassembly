...
Vendored from hillerlab/core (subworkflows/tiberius/main.nf) — re-sync by hand
if that subworkflow changes. xasm-specific deviations, all marked `ponytail:`:
  - TIBERIUS_ANNOTATION (bottom of file) is the xasm-facing wrapper. Unlike
    ANNEVO_ANNOTATION it uses `if`/`if` instead of `if`/`else if`: both
    --tiberius_annotation and --tiberius_predict may be active at once and each
    contributes a BED to the emitted channel (0, 1 or 2 elements).
  - The wrapper converts each source with its own GXF2BED alias
    (GXF2BED_TIBERIUS_FILE for the precomputed file, GXF2BED_TIBERIUS_PRED for
    the prediction) and concats the BEDs onto one channel, keeping the both-off
    channel a single Channel.empty() like ANNEVO_ANNOTATION emits.
  - The --tiberius_batch_size/--tiberius_seq_len/--tiberius_extra_args flags are
    folded into meta.tiberius_args here, mirroring the hillerlab/core outer
    workflows/tiberius/main.nf workflow.
  - TIBERIUS_MANIFEST's awk initialises `order = 0` (same fix as ANNEVO_MANIFEST:
    otherwise the first FASTA record is written with an empty order column and
    tiberiusParseManifest throws a NumberFormatException on real runs).
  - TIBERIUS_MANIFEST's stub reports `chrStub`, the record FXSPLIT's stub emits,
    so the scatter resolves under -stub-run (same fix as ANNEVO_MANIFEST).
*/
...
include { TIBERIUS_PREDICT } from '../../modules/custom/tiberius/predict/main.nf'
include { TIBERIUS_MERGE } from '../../modules/custom/tiberius/merge/main.nf'
include { GXF2BED as GXF2BED_TIBERIUS_FILE } from '../../modules/custom/gxf2bed/main'
include { GXF2BED as GXF2BED_TIBERIUS_PRED } from '../../modules/custom/gxf2bed/main'
...
/*
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    TIBERIUS_ANNOTATION — resolve the TIBERIUS annotation as BED12 file(s).

    Emits one BED per active TIBERIUS source: params.tiberius_annotation
    (precomputed file, .bed passed through, .gtf/.gff/.gff3 converted) and/or
    params.tiberius_predict (run TIBERIUS on the genome). Both flags may be set
    at once — unlike ANNEVO_ANNOTATION there is no precedence between them.
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
*/

workflow TIBERIUS_ANNOTATION {
    take:
        fasta           // channel: [ fasta ] uncompressed genome fasta

    main:
        // ponytail: concatenated incrementally rather than mixed, so the
        // both-off channel stays a single Channel.empty() — byte-for-byte the
        // shape ANNEVO_ANNOTATION emits today, and therefore a shape POLISH's
        // concat/collectFile merge is already proven against.
        ch_bed = Channel.empty()

        if (params.tiberius_annotation) {
            def source = file(params.tiberius_annotation, checkIfExists: true)

            if (source.name.endsWith('.bed')) {
                ch_bed = ch_bed.concat(Channel.value(source))
            } else {
                GXF2BED_TIBERIUS_FILE([ [ id: source.baseName ], source ])
                ch_bed = ch_bed.concat(GXF2BED_TIBERIUS_FILE.out.bed.map { _meta, bed -> bed })
            }
        }

        if (params.tiberius_predict) {
            // ponytail: flag folding mirrors the hillerlab/core outer
            // workflows/tiberius/main.nf workflow; TIBERIUS picks the flags up
            // from meta.tiberius_args.
            def flags = []
            ...
            TIBERIUS(
                fasta.map { fa -> [ [ id: fa.baseName, tiberius_args: flags.join(' ') ], fa ] },
                params.tiberius_model_cfg,
                params.tiberius_scatter,
                params.tiberius_bins,
                params.tiberius_protseq,
                params.tiberius_codingseq
            )

            GXF2BED_TIBERIUS_PRED(TIBERIUS.out.gtf)
            ch_bed = ch_bed.concat(GXF2BED_TIBERIUS_PRED.out.bed.map { _meta, bed -> bed })
        }

    emit:
        bed = ch_bed    // channel: [ bed ] — 0, 1 or 2 elements
}
